...
def resolve_initial_deposit(config):
	"""Deposito iniziale per il sizing MODE=BALANCE, da [risk] INITIAL_DEPOSIT (obbligatorio)."""
	deposit = get_setting(config, 'risk', 'INITIAL_DEPOSIT')
	logger.info(f"Deposito iniziale da config: {deposit}")
...
async def main(config_path):
	...
	mt5_client.connect()
	risk.set_initial_deposit(resolve_initial_deposit(config))
