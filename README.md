...
Con `NEWS_BLACKOUT` attivo, i tagli sono inoltre sospesi nella finestra di ±`NEWS_BLACKOUT_MINUTES` attorno agli **eventi economici ad alto impatto** sulle valute del simbolo (i "3 tori"), presi dal calendario settimanale gratuito di Forex Factory ([news_calendar.py](src/crawler/news_calendar.py)); il feed è scaricato al massimo ogni `NEWS_REFRESH_HOURS` ore e messo in cache in `news_calendar.json` accanto al config. Il blackout ferma solo i tagli della guardia — le aperture da segnale del canale non vengono mai bloccate — e in assenza di feed e cache si disattiva da solo (fail-open, con warning nel log).
