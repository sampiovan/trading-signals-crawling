...
Rispetto alla v1 non esiste più un registro ordini su file: quando un messaggio cita un ordine per prezzo (es. "CHIUDERE … (1.12500)"), il crawler lo cerca **tra le posizioni e i pending reali del conto** ([order_lookup.py](src/crawler/order_lookup.py)), con tolleranza pip-aware e best-match — e tramite il **commento dell'ordine**, che contiene il prezzo di apertura inviato dal canale arrotondato al pip (`@1.3390`, `@145.50` per le coppie JPY). Il commento è l'identificatore stabile del segnale: non cambia se il fill avviene a un livello diverso e sopravvive ai tagli/riaperture della guardia posizioni, dove accumula anche la perdita realizzata (`@1.3390 (-120)`). Sulla modifica del prezzo di un pending, il commento viene aggiornato ricreando l'ordine (MT5 non permette di cambiare il commento con una semplice modify). Il numero di decimali segue sempre il **simbolo reale dell'ordine** — ripulito dal `SYMBOL_SUFFIX` del broker, altrimenti `USDJPY.m` non verrebbe riconosciuto come coppia JPY — e mai l'asset scritto nel messaggio, che può essere sbagliato. Quando l'asset del messaggio non porta a nulla, il lookup ripiega proprio sul commento cercandolo su **tutte** le posizioni e i pending del conto (col pip del simbolo reale di ogni candidato): vale sia per un asset **inesistente** (un refuso tipo "GPS/USD" per GBP/USD) sia per un asset **esistente ma sbagliato** (es. "AUD/USD" col prezzo di una AUD/NZD, dove il simbolo si risolve ma su di esso non c'è nessun ordine). Si procede solo se il match è univoco, altrimenti il segnale viene scartato — e ogni scarto per ordine non trovato arriva come notifica nei Saved Messages, non solo nel log, comprese le singole posizioni saltate di una chiusura multipla riuscita solo in parte. Il ripiego vale però solo per i messaggi che **cercano** un ordine esistente: la deduplica del catch-up, che dopo un riavvio verifica se un'apertura era già stata eseguita, lo esclude di proposito (lì la domanda è "è *esattamente* questo ordine?", e un ordine di un'altra coppia non è mai la risposta giusta).
...
| `open` | Notifica che un pending è stato riempito: si verifica soltanto che il pending sia diventato posizione, mai un'apertura a mercato (sarebbe un duplicato). Arriva spesso come **risposta Telegram** al placement, il cui prezzo identifica l'ordine anche se quello del messaggio è storpiato; se il pending non si trova, il segnale è scartato con alert | `Ordine Buy EUR/USD Aperto  Prezzo di ingresso 1.12500` |
| `modify` | Modifica del prezzo di ingresso di un pending (o del solo SL se mirata via reply). In `MODE=BALANCE` il ripiazzo ricalcola anche la size sul balance attuale: se ci sono posizioni aperte sull'asset può solo salire (mai sotto i lotti piazzati), altrimenti si ricalcola pieno anche al ribasso (evita di sovraesporsi con lotti troppo grandi per un conto sceso) | `(BUY LIMIT EUR/USD) - MODIFICARE IL PREZZO DI INGRESSO DA … A …` |
