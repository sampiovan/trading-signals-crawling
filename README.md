...
| `modify` | Modifica del prezzo di ingresso di un pending (o del solo SL se mirata via reply). In `MODE=BALANCE` il ripiazzo aggiorna anche la size al valore consentito dal balance attuale, ma solo verso l'alto (se il conto è cresciuto): mai al di sotto dei lotti con cui era stato piazzato | `(BUY LIMIT EUR/USD) - MODIFICARE IL PREZZO DI INGRESSO DA … A …` |
